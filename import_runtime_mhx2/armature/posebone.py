...
def addPoseBone(amt, bone, customShape, boneGroup, locArg, lockRot, lockScale, ik_dof, poseFlags, constraints):
    try:
    ...
    if boneGroup:
        ...
        fp.add("    bone_group Refer BoneGroup %s ;\n" % boneGroup)
...
"    ik_min Array %.4f %.4f %.4f ; \n" % (xmin, ymin, zmin))
    ...
    if ik_stiff:
        ...
        fp.add("    ik_stiffness  Array %.4f %.4f %.4f ;\n" % (ik_stiff_x, ik_stiff_y, ik_stiff_z))
    else:
        fp.add("    ik_stiffness Array 0.0 0.0 0.0  ; \n")

    if customShape:
        fp.add("    custom_shape Refer Object %s ; \n" % customShape)

    rotMode = getRotationMode(poseFlags, amt.options)
    fp.add("    rotation_mode '%s' ;\n" % rotMode)
