...
class Pose:
    ...
    def __init__(self, human, options):
        ...
        debugCoords("Pose1")
        amt.setup()
        print("Head %s" % amt.bones["head"].head)
    ...
    def display(self):
        print("<Pose %s", self.armature.name)
            ...
            for cns in pb.constraints:
                ...
                for bname in bnames:
                    if bname:
                        ...
                        if not target.dirty:
                            print("Dirty %s before %s" % (pb.name, target.name))
                            dirty = True
        if dirty:
    ...
    def readBvhFile(self, filepath):
        ...
        for pb, offset, channels, isRoot in bones:
            ...
            if pb.name in []:
                print("%s %s", pb.name, order)
                print("%s", str(channels))
                print("%s %s %s", ax/D, ay/D, az/D)
                print("R %s", pb.bone.matrixRest)
                print("M1 %s", mat1)
                print("M2 %s", mat2)
                print("P %s", pb.matrixPose)
                print("G %s", pb.matrixGlobal)
...
class PoseBone:
    ...
    def build(self):
        ...
        try:
            ...
            print("%s", self.bone.matrixRest)
            print("%s", self.matrixPose)
            print("%s", self.matrixGlobal)
            halt
    ...
    def stretchTo(self, goal, doStretch):
        ...
        if 0 and self.name in ["DfmKneeBack_L", "DfmLoLeg_L"]:
            print("Stretch %s", self.name)
            print("G %s", goal)
            print("M1 %s", self.matrixGlobal)
            print("P1 %s", pose)
    ...
    def printMats(self):
        print(self.name)
        print("H4 %s", self.head4)
        print("T4 %s", self.tail4)
        print("RM %s", self.bone.matrixRest)
        print("RV %s", np.dot(self.bone.matrixRest, self.yvector4))
        print("P %s", self.matrixPose)
        print("Rel %s", self.bone.matrixRelative)
        print("G %s", self.matrixGlobal)
        print("GV %s", np.dot(self.matrixGlobal, self.yvector4))
